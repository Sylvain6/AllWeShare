...
use App\Service\AnalyzeContent;
...
class PostController extends AbstractController
{
     ...
     */
    public function new(Request $request, PostRepository $postRepository, GroupRepository $groupRepository, PaginationService $paginationService, AnalyzeContent $analyzeContent): Response
    {
        $post = new Post();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
             ...
             */
            $res = $analyzeContent->analyzeContent(strtolower($post->getTitle()));
            if ($res !== 'undefined') {
                $post->setTypePost($res);
            } else {
                $res =$analyzeContent->analyzeContent($post->getDescription());
                $post->setTypePost($res);
            }
        ...
        $posts = $paginationService->finWithPagination(
            $page,
            $postRepository,
            null
        );
     ...
     */
    public function edit(Request $request, Post $post, AnalyzeContent $analyzeContent): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            * Analyse du texte afin de detecter le type du compte
            */
            $res = $analyzeContent->analyzeContent($post->getTitle());
            if ($res !== 'undefined') {
                $post->setTypePost($res);
            } else {
                $res = $analyzeContent->analyzeContent($post->getDescription());
